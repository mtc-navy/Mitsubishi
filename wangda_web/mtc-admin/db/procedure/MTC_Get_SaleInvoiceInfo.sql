DROP PROCEDURE "MTC_Get_SaleInvoiceInfo";
CREATE PROCEDURE "MTC_Get_SaleInvoiceInfo"
(
  DocNum INTEGER,
  IsMaster NVARCHAR(10), -- Y:查询主数据 N：查询明细数据
  FromOrder NVARCHAR(10) DEFAULT 'N'  --Y:是 N：否
)
LANGUAGE SQLSCRIPT
AS
BEGIN
    ...
    --查询主数据
    IF :IsMaster=N'Y' AND :FromOrder=N'N' THEN
    BEGIN
      SELECT
        --表头信息
        ...
        T0."DocNum" AS "InvDocNum",--发货单号
        T0."U_FId" AS "fid",--内联交易号
    ...
    --查询明细数据
    IF :IsMaster=N'N' AND :FromOrder=N'N' THEN
    BEGIN
      SELECT
        T0."LineNum",--行号
        T0."ItemCode",--物料代码
    ...
    END IF;
     ...
     --查询主数据
    IF :IsMaster=N'Y' AND :FromOrder=N'Y' THEN
    BEGIN
      SELECT
        --表头信息
        T0."DocEntry",--DocEntry
    ...
    --查询明细数据
    IF :IsMaster=N'N' AND :FromOrder=N'Y' THEN
    BEGIN
      SELECT
        T0."LineNum",--行号
        T0."ItemCode",--物料代码
