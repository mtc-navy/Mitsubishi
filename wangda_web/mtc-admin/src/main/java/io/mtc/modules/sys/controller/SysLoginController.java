...
import io.mtc.common.config.KaptchaConfig;
...
public class SysLoginController {
	...
	public R login(String username, String password, String captcha) {
		...
		/**
		 * 尝试连接查询订单，用于用户每次登录后的初次连接尝试
		 * 用于优化ServiceLayer的初次访问性能
		 */
		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					QueryParam queryParam = new QueryParam(new QueryParam.Builder());
					queryParam.setTop(1);
					ordersDao.queryList(Order.class, queryParam);
				}catch(Exception ex){}
			}
		}).start();
