$(function () {
    $("#jqGrid").setjgGrid({api: 'sys/common/userdept', hiddenFields: ['DOCENTRY'], buttons: []});
    $("#jqGridList").setjgGridList({api:'sys/verHis/userdept', hiddenFields:['docEntry'], ischeck:true});
    $("#jqGriddiff").setjgGridList({api:'sys/verDiff/userdept', hiddenFields:['docEntry','isAdd'], ischeck:false,isred:true});
...
/*"param1:预算版本（下拉选择，传Code）
param2:公司（下拉选择，传Code）
param3:部门（下拉选择，传Code）
param4：经营单元（下拉选择，传Code）
param5：销售单元（下拉选择，传Code）
param6：开始日期
param7：结束日期
param8：状态"*/
...
var vm = new Vue({
    ...
    components: {
        'bud-table': buduserdeptComponent,
    },
    methods: {
        query: function () {
        ...
        saveOrUpdate: function (event) {
            ...
            var url="budgbase/userdept/add";
            if(vm.info.docentry)
            {
               //修改
                url="budgbase/userdept/edit";
            }
         /*   $.ajax({
                type: "POST",
                url: baseURL + url,
        ...
        getInfo: function (id) {
            ...
            httpGetInfo("budgbase/userdept/view", {
                docEntry: id
            }, function (r) {
                if (r.code === 0) {
        ...
        postapprove: function (event) {
            var url="budgbase/userdept/postApprove";
        ...
        approve: function (event) {
            var url="budgbase/userdept/approve";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        reject: function (event) {
            var url="budgbase/userdept/reject";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        close: function (event) {
           var url="budgbase/userdept/close";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        cancelclose: function (event) {
            var url="budgbase/userdept/cancelClose";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        copyfrom: function (event) {
            ...
            $.ajax({
                type: "POST",
                url: baseURL + "budgbase/userdept/copyFrom",
                contentType: "application/json",
           ...
           /* var url="budgbase/userdept/copyFrom";
            httpPostFormData(url,{
                main:vm.info
            },function(r){
                if (r.code === 0) {
