$(function () {
    $("#jqGrid").setjgGrid({api: 'sys/common/apprcompany', hiddenFields: ['DOCENTRY'], buttons: []});
    $("#jqGridList").setjgGridList({api:'sys/verHis/companyApprove', hiddenFields:['docEntry'], ischeck:true});
    $("#jqGriddiff").setjgGridList({api:'sys/verDiff/companyApprove', hiddenFields:['docEntry','isAdd'], ischeck:false,isred:true});
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            ...
            var mtcBudCompanyEntity=vm.info;
            mtcBudCompanyEntity.docdate=GetYMD(vm.info.docdate);
            var url="budgbase/apprcompany/add";
            if(vm.info.docentry)
            {
               //修改
                url="budgbase/apprcompany/edit";
            }
            httpPost(url,{
                mtcBudCompanyEntity:mtcBudCompanyEntity,mtcBudCompany1EntityList:vm.Buds
            },function(r){
                if (r.code === 0) {
        ...
        getInfo: function (id) {
            ...
            httpGetInfo("budgbase/apprcompany/view", {
                docEntry: id
            }, function (r) {
                if (r.code === 0) {

                    vm.info=r.apprCompany.mtcBudCompanyEntity;
                    vm.Buds=r.apprCompany.mtcBudCompany1EntityList;
                    vm.drop.stateList = Document.initStatusList();
                    vm.drop.verList = Document.initVerList();
                    Document.initCompList(vm.orgLevel,function(result){
                        vm.drop.compList = result;
        ...
        postapprove: function (event) {
            var url="budgbase/apprcompany/postapprove";
        ...
        approve: function (event) {
            var url="budgbase/apprcompany/approve";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        reject: function (event) {
            var url="budgbase/apprcompany/reject";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        close: function (event) {
           var url="budgbase/apprcompany/close";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        cancelclose: function (event) {
            var url="budgbase/apprcompany/cancelclose";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        genfrom: function (event) {
            ...
            if(isgen)
            {
                var url="budgbase/apprcompany/genfrom";
                httpPostFormData(url,{
                    ...
                    if (r.code === 0) {
                        alert('操作成功', function () {
                            vm.drop.itemList=r.apprCompany;
                            vm.initBuds();
                        });
                    } else {
                        alert(r.msg);
        ...
        exportDetail:function(){
            ...
            Document.exportDetail('sys/common/clickCompany',param);
        },
