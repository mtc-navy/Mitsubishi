$(function () {
    $("#jqGrid").setjgGrid({api: 'sys/common/apprcomarea', hiddenFields: ['DOCENTRY'], buttons: []});
    $("#jqGridList").setjgGridList({api:'sys/verHis/comareaApprove', hiddenFields:['docEntry'], ischeck:true});
    $("#jqGriddiff").setjgGridList({api:'sys/verDiff/comareaApprove', hiddenFields:['docEntry','isAdd'], ischeck:false,isred:true});
...
var vm = new Vue({
    ...
    methods: {
        ...
        add: function () {
            ...
            Document.initComareList(vm.orgLevel,function(result){
        ...
        saveOrUpdate: function (event) {
            ...
            // console.log(data)
            var mtcBudComAearEntity=vm.info;
            mtcBudComAearEntity.docdate=GetYMD(vm.info.docdate);
            var url="budgbase/apprcomaear/add";
            if(vm.info.docentry)
            {
               //修改
                url="budgbase/apprcomaear/edit";
            }
            httpPost(url,{
                mtcBudComAearEntity:mtcBudComAearEntity,mtcBudComAear1EntityList:vm.Buds
            },function(r){
                if (r.code === 0) {
        ...
        getInfo: function (id) {
            ...
            httpGetInfo("budgbase/apprcomaear/view", {
                docEntry: id
            }, function (r) {
                if (r.code === 0) {

                    vm.info=r.apprcomaear.mtcBudComAearEntity;
                    vm.Buds=r.apprcomaear.mtcBudComAear1EntityList;
                    vm.drop.stateList = Document.initStatusList();
                    vm.drop.verList = Document.initVerList();
                    Document.initComareList(vm.orgLevel,function(result){
                        vm.drop.compList = result;
        ...
        postapprove: function (event) {
            var url="budgbase/apprcomaear/postapprove";
        ...
        approve: function (event) {
            var url="budgbase/apprcomaear/approve";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        reject: function (event) {
            var url="budgbase/apprcomaear/reject";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        close: function (event) {
           var url="budgbase/apprcomaear/close";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        cancelclose: function (event) {
            var url="budgbase/apprcomaear/cancelclose";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        genfrom: function (event) {
            ...
            if(isgen)
            {
                var url="budgbase/apprcomaear/genfrom";
                httpPostFormData(url,{
                    VERCODE:vm.info.vercode
                    ,COMAREA:vm.info.comareaid

                },function(r){
                    if (r.code === 0) {
                        alert('操作成功', function () {
                            vm.drop.itemList=r.apprcomaear;
                            vm.initBuds();
                        });
                    } else {
                        alert(r.msg);
        ...
        exportDetail:function(){
            ...
            Document.exportDetail('sys/common/clickComArea',param);
        },
