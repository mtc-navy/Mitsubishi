$(function () {
    $("#jqGrid").setjgGrid({api: 'sys/common/apprbuziunit', hiddenFields: ['DOCENTRY'], buttons: []});
    $("#jqGridList").setjgGridList({api:'sys/verHis/buzUnitApprove', hiddenFields:['docEntry'], ischeck:true});
    $("#jqGriddiff").setjgGridList({api:'sys/verDiff/buzUnitApprove', hiddenFields:['docEntry','isAdd'], ischeck:false,isred:true});
...
var vm = new Vue({
    ...
    methods: {
            ...
            Document.initCompList(vm.orgLevel,function(result){
                ...
                if(vm.treeNode.companyCode) {
                    ...
                    vm.selectedcompcode(vm.info.bplid,function(){
                        ...
                        vm.selectedbuzicode(vm.info.buziunitcode)
                    });
                }
            });
        ...
        saveOrUpdate: function (event) {
            var mtcBudBuziUnitEntity=vm.info;
            mtcBudBuziUnitEntity.docdate=GetYMD(vm.info.docdate);

        var url="budgbase/apprbuziunit/add";
            if(vm.info.docentry)
            {
               //修改
                url="budgbase/apprbuziunit/edit";
            }
            httpPost(url,{
                mtcBudBuziUnitEntity:mtcBudBuziUnitEntity,mtcBudBuziUnit1EntityList:vm.Buds
            },function(r){
                if (r.code === 0) {
        ...
        getInfo: function (id) {
            ...
            httpGetInfo("budgbase/apprbuziunit/view", {
                docEntry: id
            }, function (r) {
                if (r.code === 0) {


                    vm.info=r.apprbuziunit.mtcBudBuziUnitEntity;
                    vm.Buds=r.apprbuziunit.mtcBudBuziUnit1EntityList;
                    vm.drop.stateList = Document.initStatusList();
                    vm.drop.verList = Document.initVerList();
                    Document.initCompList(vm.orgLevel,function(result){
                        vm.drop.compList = result;
        ...
        postapprove: function (event) {
            var url="budgbase/apprbuziunit/postapprove";
        ...
        approve: function (event) {
            var url="budgbase/apprbuziunit/approve";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        reject: function (event) {
            var url="budgbase/apprbuziunit/reject";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        close: function (event) {
           var url="budgbase/apprbuziunit/close";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        cancelclose: function (event) {
            var url="budgbase/apprbuziunit/cancelclose";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        genfrom: function (event) {
            ...
            if(isgen)
            {
                var url="budgbase/apprbuziunit/genfrom";
                httpPostFormData(url,{
                    ...
                    if (r.code === 0) {
                        alert('操作成功', function () {
                            vm.drop.itemList=r.apprbuziunit;
                            vm.initBuds();
                        });
                    } else {
                        alert(r.msg);
        ...
        exportDetail:function(){
            ...
            Document.exportDetail('sys/common/clickBuziUnit',param);
        },
