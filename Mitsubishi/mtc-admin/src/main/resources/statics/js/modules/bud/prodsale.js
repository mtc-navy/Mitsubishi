$(function () {
    $("#jqGrid").setjgGrid({api: 'sys/common/prodsale', hiddenFields: ['DOCENTRY'], buttons: []});
    $("#jqGridList").setjgGridList({api:'sys/verHis/prodsale', hiddenFields:['docEntry'], ischeck:true});
    $("#jqGriddiff").setjgGridList({api:'sys/verDiff/prodsale', hiddenFields:['docEntry','isAdd'], ischeck:false,isred:true});
    Document.initDeptTree(null,vm.orgLevel,"T");
    vm.initqdrop();
    vm.showTreeData();
    vm.isAddTotal=true;//明细列表添加行合计
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            ...
            var url="budgbase/prodsale/add";
            if(vm.info.docentry)
            {
               //修改
                url="budgbase/prodsale/edit";
            }
            httpPost(url,{
        ...
        getInfo: function (id) {
            ...
            httpGetInfo("budgbase/prodsale/view", {
                docEntry: id
            }, function (r) {
                if (r.code === 0) {
        ...
        reload: function (event) {
          ...
          //  console.log(vm.q);
            ...
            // var rw = rightDiv.clientWidth || rightDiv.offsetWidth;
            // var lw=leftDiv.clientWidth || leftDiv.offsetWidth;
            // if(rw==0) $("#jqGrid").setGridWidth($(window).width()-lw-20);
        ...
        postapprove: function (event) {
            var url="budgbase/prodsale/postApprove";
        ...
        approve: function (event) {
            var url="budgbase/prodsale/approve";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        reject: function (event) {
            var url="budgbase/prodsale/reject";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        close: function (event) {
           var url="budgbase/prodsale/close";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        cancelclose: function (event) {
            var url="budgbase/prodsale/cancelClose";
            httpPostFormData(url,{
                docEntry: vm.info.docentry
            },function(r){
                if (r.code === 0) {
        ...
        copyfrom: function (event) {
            ...
            $.ajax({
                type: "POST",
                url: baseURL + "budgbase/prodsale/copyFrom",
